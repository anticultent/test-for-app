...
import {
  ...
  Video, Sparkles, MicOff, Play, Headphones, BookOpen,
} from "lucide-react";
...
const SETS = [
  { id: 201, artist: "DJ Amaranth", handle: "@djamaranth", title: "Midnight Circuit", genre: "House", duration: 3624, plays: 18400, g: GRADIENTS[3] },
  { id: 202, artist: "Sable & Sax", handle: "@sableandsax", title: "Blue Hour Fusion", genre: "Jazz Fusion", duration: 2760, plays: 6200, g: GRADIENTS[4] },
  { id: 203, artist: "Kilowatt Kid", handle: "@kilowattkid", title: "Glitch Parade", genre: "Hyperpop", duration: 1980, plays: 11200, g: GRADIENTS[1] },
  { id: 204, artist: "Obi & The Drift", handle: "@obithedrift", title: "Diaspora Nights", genre: "Afrobeat", duration: 3120, plays: 8700, g: GRADIENTS[5] },
  { id: 205, artist: "Two Rivers", handle: "@tworivers", title: "Porch Light Sessions", genre: "Indie Folk", duration: 2340, plays: 3100, g: GRADIENTS[2] },
];

const PODCASTS = [
  { id: 401, show: "Bedroom Sessions", title: "Ep. 12 — Turning a Closet Into a Studio", hosts: ["Nova Resin"], guest: "Glass Feather", category: "Artist Interviews", duration: 2760, plays: 5400, g: GRADIENTS[0] },
  { id: 402, show: "The Mix Down", title: "Ep. 4 — Why B2B Sets Are Back", hosts: ["DJ Amaranth", "Sable & Sax"], guest: null, category: "Music Culture", duration: 3300, plays: 9100, g: GRADIENTS[3] },
  { id: 403, show: "Behind the Boards", title: "Ep. 7 — Vocal FX Deep Dive", hosts: ["Kilowatt Kid"], guest: "Obi & The Drift", category: "Production", duration: 2100, plays: 4200, g: GRADIENTS[1] },
  { id: 404, show: "Setlist Live", title: "Ep. 1 — Launch Day Roundtable", hosts: ["Two Rivers", "Sable & Sax", "Obi & The Drift"], guest: null, category: "Setlist Originals", duration: 3600, plays: 12800, g: GRADIENTS[5] },
];

const LISTENERS_SEED = ["riverwalks", "juno_static", "priya.k", "coastwise", "loafbread"];

const BLOG_POSTS = [
  {
    id: 301, tag: "Artist Spotlight", title: "Inside Nova Resin's Bedroom Studio", author: "Setlist Editorial", date: "Aug 12", readMins: 4, g: GRADIENTS[0],
    excerpt: "How a closet, a laptop, and a $40 mic built a following of 40,000.",
    body: "Nova Resin's entire setup fits on a desk built into a converted closet — an interface, a pair of headphones, and a microphone that cost less than a night out. What it lacks in gear it makes up for in repetition: a new voice memo idea nearly every day, most of which never leave the phone.\n\nThe breakout clip, 'Static Bloom,' was recorded in nine takes on a Tuesday. No producer, no studio time booked. Just a closet, a chorus that wouldn't leave, and the willingness to post something unfinished.",
  },
  {
    id: 302, tag: "Production", title: "5 Vocal FX Every Bedroom Producer Should Know", author: "Setlist Editorial", date: "Aug 5", readMins: 6, g: GRADIENTS[2],
    excerpt: "Reverb and echo are just the start — here's how to actually use them.",
    body: "Reverb sells a room; echo sells a feeling. The two get reached for interchangeably, but they do different jobs — reverb answers 'where is this happening,' echo answers 'what does this moment mean.' Chorus, meanwhile, is the cheapest way to make one voice sound like three.\n\nRing modulation (the 'robot' effect) is the one people reach for too early. Used sparingly on a single line, it's a great accent. Used on a whole verse, it stops sounding like a choice and starts sounding like a limitation.",
  },
  {
    id: 303, tag: "Culture", title: "The Return of the B2B Set", author: "Setlist Editorial", date: "Jul 29", readMins: 5, g: GRADIENTS[3],
    excerpt: "Why more DJs are choosing to share the decks instead of solo sets.",
    body: "Back-to-back sets used to be a festival-only novelty. Now they're showing up in bedrooms and small clubs alike, partly because going live together removes the loneliest part of DJing: the four hours between when you start prepping and when anyone's actually listening.\n\nThe format also rewards a different skill — reading another DJ in real time, not just reading a crowd. The best b2b sets sound like a conversation, not two people taking turns.",
  },
  {
    id: 304, tag: "Product", title: "Setlist's Co-Streaming, Explained", author: "Setlist Team", date: "Jul 20", readMins: 3, g: GRADIENTS[1],
    excerpt: "A look at what's actually happening under the hood when you go live together.",
    body: "When you invite a co-host, both of you publish audio and video into the same room, and everyone watching receives a mixed version in real time — the same basic architecture video call apps use, tuned for lower latency since musical timing doesn't forgive lag the way conversation does.\n\nVocal and video FX run on your own device before your stream leaves it, which is why they respond instantly instead of a beat behind.",
  },
];
...
const SET_BPM = { House: 124, Hyperpop: 150, "Jazz Fusion": 96, Afrobeat: 110, "Bedroom Pop": 100, Synthwave: 108, "Indie Folk": 92 };

// Generative audio "preview" for DJ sets — since there's no real uploaded
// audio to stream, this synthesizes a short genre-tempo'd loop live via
// Web Audio instead of faking a play button that produces nothing.
function useTonePreview() {
  ...
  const toggle = (set) => {
    if (playingId === set.id) { stop(); return; }
    stop();
    const ctx = ensureCtx();
    const root = 220 * Math.pow(2, (hashCode(set.genre) % 12) / 12);
    const scaleSteps = [0, 3, 5, 7, 10];
    const bpm = SET_BPM[set.genre] || 110;
    const stepSec = 60 / bpm / 2;
    const wave = set.genre === "Hyperpop" ? "square" : set.genre === "House" ? "sawtooth" : "triangle";
    const master = ctx.createGain();
    master.gain.value = 0.12;
    master.connect(ctx.destination);
    let i = 0;
    intervalRef.current = setInterval(() => {
      const semis = scaleSteps[i % scaleSteps.length] + Math.floor(i / scaleSteps.length) % 2 * 12;
...
function SideNav({ screen, setScreen, onUpload }) {
  const items = [
    ...
    { key: "sets", icon: Headphones, label: "Sets" },
...
function Waveform({ bars = 24, height = 28, color = "#F3EFEA", opacity = 0.9, gap = 3, playing = true }) {
      ...
      {heights.map((h, i) => (
        <div
          ...
          style={{
            ...
            borderRadius: 2,
            background: color,
            opacity: playing ? opacity : opacity * 0.4,
            height: `${h * 100}%`,
...
function TopBar({ title, onSearch, onBlog }) {
  return (
    <div
      style={{
        display: "flex",
    ...
    >
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
        {onBlog && (
          <button onClick={onBlog} style={{ background: "none", border: "none", cursor: "pointer", padding: 4 }}>
            <BookOpen size={18} color="#948FA3" />
          </button>
        )}
...
function FeedScreen({ liked, onLike, onSearch, onBlog, onComment, onRemix, commentsState }) {
  return (
    <div style={{ flex: 1, minHeight: 0, display: "flex", flexDirection: "column" }}>
      <TopBar title="SETLIST" onSearch={onSearch} onBlog={onBlog} />
...
function LiveScreen({ onOpen, onSearch, onBlog }) {
  const [genre, setGenre] = useState("All");
  const filtered = genre === "All" ? LIVES : LIVES.filter((l) => l.genre === genre);
  return (
    <div style={{ flex: 1, minHeight: 0, display: "flex", flexDirection: "column" }}>
      <TopBar title="LIVE" onSearch={onSearch} onBlog={onBlog} />
...
/* ---------- DJ Sets ---------- */
function SetCard({ set, playing, onToggle }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 4px", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
      <GelCard g={set.g} style={{ width: 52, height: 52, borderRadius: 10, flexShrink: 0 }}>
        <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1 }}>
          <Headphones size={16} color="rgba(13,11,20,0.55)" />
        </div>
      </GelCard>
      <div style={{ flex: 1, minWidth: 0 }}>
        <p style={{ margin: 0, fontFamily: "Inter", fontWeight: 600, fontSize: 13, color: "#F3EFEA", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{set.title}</p>
        <p style={{ margin: "2px 0 0", fontFamily: "Inter", fontSize: 11.5, color: "#948FA3" }}>{set.artist} · {set.genre}</p>
        <p style={{ margin: "2px 0 0", fontFamily: "'JetBrains Mono', monospace", fontSize: 10, color: "#6b6578" }}>{fmtTime(set.duration)} · {fmtNum(set.plays)} plays</p>
      </div>
      <button
        onClick={() => onToggle(set)}
        style={{ width: 36, height: 36, borderRadius: 999, border: "none", background: playing ? GELS.violet : "rgba(255,255,255,0.08)", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", flexShrink: 0 }}
      >
        {playing ? <Square size={12} color="#0D0B14" fill="#0D0B14" /> : <Play size={13} color="#F3EFEA" fill="#F3EFEA" style={{ marginLeft: 2 }} />}
      </button>
    </div>
  );
}

function PodcastCard({ ep, playing, onToggle }) {
  const speakers = ep.guest ? [...ep.hosts, ep.guest] : ep.hosts;
  return (
    <div style={{ padding: "12px 4px", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
        <GelCard g={ep.g} style={{ width: 52, height: 52, borderRadius: 10, flexShrink: 0 }}>
          <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1 }}>
            <Mic2 size={16} color="rgba(13,11,20,0.55)" />
          </div>
        </GelCard>
        <div style={{ flex: 1, minWidth: 0 }}>
          <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 9, letterSpacing: 0.5, color: GELS.blue, fontWeight: 600 }}>{ep.show.toUpperCase()}</span>
          <p style={{ margin: "3px 0 0", fontFamily: "Inter", fontWeight: 600, fontSize: 13, color: "#F3EFEA", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{ep.title}</p>
          <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 4 }}>
            <HostStack names={speakers} size={15} />
            <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 9.5, color: "#6b6578" }}>{fmtTime(ep.duration)} · {fmtNum(ep.plays)} plays</span>
          </div>
        </div>
        <button
          onClick={() => onToggle(ep)}
      ...
      {playing && (
        <div style={{ marginTop: 8, paddingLeft: 64 }}>
          <Waveform bars={20} height={14} color={GELS.violet} opacity={0.7} playing={playing} />
        </div>
      )}
    </div>
  );
...
function SetsScreen({ onSearch, onBlog, onStartPodcast }) {
  const [tab, setTab] = useState("sets");
  const [genre, setGenre] = useState("All");
  const [podPlayingId, setPodPlayingId] = useState(null);
  const { playingId, toggle } = useTonePreview();
  const genres = ["All", ...Array.from(new Set(SETS.map((s) => s.genre)))];
  const filtered = genre === "All" ? SETS : SETS.filter((s) => s.genre === genre);
  const togglePod = (ep) => setPodPlayingId((id) => (id === ep.id ? null : ep.id));

  return (
    <div style={{ flex: 1, minHeight: 0, display: "flex", flexDirection: "column" }}>
      <TopBar title={tab === "sets" ? "SETS" : "PODCASTS"} onSearch={onSearch} onBlog={onBlog} />
      <div style={{ display: "flex", gap: 8, padding: "0 16px 12px", flexShrink: 0 }}>
        <button onClick={() => setTab("sets")} style={{ ...toggleChipStyle(tab === "sets", GELS.violet), flex: 1, textAlign: "center" }}>DJ Sets</button>
        <button onClick={() => setTab("podcasts")} style={{ ...toggleChipStyle(tab === "podcasts", GELS.violet), flex: 1, textAlign: "center" }}>Podcasts</button>
      </div>
      ...
      <div style={{ flex: 1, minHeight: 0, overflowY: "auto" }} className="no-scrollbar">
        {tab === "sets" ? (
          <>
            <div style={{ display: "flex", gap: 8, overflowX: "auto", padding: "0 16px 14px" }} className="no-scrollbar">
              {genres.map((g) => (
                <button key={g} onClick={() => setGenre(g)} style={toggleChipStyle(genre === g, GELS.violet)}>
                  {g}
                </button>
              ))}
            </div>
            <div style={{ padding: "0 16px 8px" }}>
              {filtered.map((s) => (
                <SetCard key={s.id} set={s} playing={playingId === s.id} onToggle={toggle} />
              ))}
            </div>
            <p style={{ fontFamily: "Inter", fontSize: 11, color: "#6b6578", textAlign: "center", padding: "10px 24px 24px", lineHeight: 1.5 }}>
              Previews are a live-generated audio sketch, not the real recording — actual playback needs uploaded audio and a media backend.
            </p>
          </>
        ) : (
          <>
            <div style={{ padding: "0 16px 10px" }}>
              <button
                ...
                style={{ width: "100%", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, padding: "11px 14px", borderRadius: 12, border: "none", cursor: "pointer", background: `linear-gradient(135deg, ${GELS.amber}, ${GELS.blue})`, color: "#0D0B14", fontFamily: "Inter", fontWeight: 700, fontSize: 13 }}
              >
                <Mic size={15} /> Start a podcast room
              </button>
            </div>
            <div style={{ padding: "0 16px 8px" }}>
              {PODCASTS.map((ep) => (
                <PodcastCard key={ep.id} ep={ep} playing={podPlayingId === ep.id} onToggle={togglePod} />
              ))}
            </div>
            <p style={{ fontFamily: "Inter", fontSize: 11, color: "#6b6578", textAlign: "center", padding: "10px 24px 24px", lineHeight: 1.5 }}>
              Playback here is a visual preview only — real episode audio needs uploaded files and a media backend, same as everywhere else in this prototype.
            </p>
          </>
        )}
      </div>
  ...
  return (
    <div style={{ flex: 1, minHeight: 0, display: "flex", flexDirection: "column" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "14px 16px", flexShrink: 0 }}>
        ...
        <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 11, letterSpacing: 0.5, color: "#948FA3" }}>{post.tag.toUpperCase()}</span>
      </div>
      <div style={{ flex: 1, minHeight: 0, overflowY: "auto", padding: "0 20px 30px" }} className="no-scrollbar">
        <GelCard g={post.g} style={{ height: 150, borderRadius: 14, marginBottom: 16 }} />
        <h1 style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 27, letterSpacing: 0.5, color: "#F3EFEA", margin: "0 0 6px", lineHeight: 1.05 }}>{post.title}</h1>
        <p style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10.5, color: "#948FA3", margin: "0 0 18px" }}>{post.author} · {post.date} · {post.readMins} min read</p>
        {post.body.split("\n\n").map((para, i) => (
          <p key={i} style={{ fontFamily: "Inter", fontSize: 14, lineHeight: 1.7, color: "#E7E3EE", margin: "0 0 14px" }}>{para}</p>
        ))}
      </div>
    </div>
  );
...
function BlogScreen() {
  ...
  if (selected) return <BlogPostDetail post={selected} onBack={() => setSelected(null)} />;
  return (
    <div style={{ flex: 1, minHeight: 0, display: "flex", flexDirection: "column" }}>
      <TopBar title="BLOG" onSearch={() => {}} />
      <div style={{ flex: 1, minHeight: 0, overflowY: "auto", padding: "4px 16px 24px" }} className="no-scrollbar">
        {BLOG_POSTS.map((p) => (
          <button
            key={p.id}
            onClick={() => setSelected(p)}
            style={{ display: "block", width: "100%", textAlign: "left", background: "none", border: "none", cursor: "pointer", padding: "12px 0", borderBottom: "1px solid rgba(255,255,255,0.06)" }}
          >
            <div style={{ display: "flex", gap: 12 }}>
              <GelCard g={p.g} style={{ width: 64, height: 64, borderRadius: 10, flexShrink: 0 }} />
              <div style={{ flex: 1, minWidth: 0 }}>
                <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 9, letterSpacing: 0.5, color: GELS.violet, fontWeight: 600 }}>{p.tag.toUpperCase()}</span>
                <p style={{ margin: "5px 0 0", fontFamily: "Inter", fontWeight: 700, fontSize: 13.5, color: "#F3EFEA", lineHeight: 1.25 }}>{p.title}</p>
                <p style={{ margin: "4px 0 0", fontFamily: "Inter", fontSize: 11.5, color: "#948FA3", overflow: "hidden", textOverflow: "ellipsis", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical" }}>{p.excerpt}</p>
                <p style={{ margin: "6px 0 0", fontFamily: "'JetBrains Mono', monospace", fontSize: 9.5, color: "#6b6578" }}>{p.author} · {p.date} · {p.readMins} min read</p>
              </div>
            </div>
          </button>
        ))}
    ...
    <div style={{ flex: 1, minHeight: 0, display: "flex", flexDirection: "column", overflowY: "auto" }} className="no-scrollbar">
      ...
      <div style={{ padding: "0 20px 10px", display: "flex", flexDirection: "column", alignItems: "center" }}>
        ...
        <div style={{ display: "flex", gap: 22, marginBottom: 16 }}>
          {[["Uploads", String(38 + uploads.length)], ["Followers", "12.4k"], ["Following", "204"]].map(([label, val]) => (
      ...
      <div style={{ padding: "6px 16px 0" }}>
        <p style={sectionLabel}>YOUR MUSIC ({uploads.length + 9})</p>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 6, padding: "0 16px 24px" }}>
        {uploads.map((u) => (
          <GelCard key={u.id} g={u.g} style={{ borderRadius: 8, aspectRatio: "0.72", display: "flex", flexDirection: "column", justifyContent: "flex-end", padding: 6 }}>
            <div style={{ position: "relative", zIndex: 1 }}>
              {u.kind === "set" ? <Headphones size={11} color="#0D0B14" /> : <Music2 size={11} color="#0D0B14" />}
              <p style={{ margin: "3px 0 0", fontFamily: "Inter", fontSize: 9.5, fontWeight: 700, color: "#0D0B14", lineHeight: 1.15, overflow: "hidden", textOverflow: "ellipsis", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical" }}>{u.title}</p>
            </div>
          </GelCard>
        ))}
...
function UploadModal({ onClose, onGoLive, onPodcast, onPublish }) {
  ...
  const publish = () => {
    onPublish({ kind, title: title.trim() || (kind === "set" ? "Untitled Set" : "Untitled Clip") });
    setStep("done");
  };

  const headerText = { pick: "SHARE SOMETHING", form: kind === "set" ? "NAME YOUR SET" : "NAME YOUR CLIP", done: "PUBLISHED" }[step];
    ...
    <div style={{ position: "absolute", inset: 0, background: "rgba(13,11,20,0.7)", backdropFilter: "blur(4px)", display: "flex", alignItems: "flex-end", zIndex: 10 }} onClick={onClose}>
      <div onClick={(e) => e.stopPropagation()} style={{ width: "100%", background: "#17141F", borderRadius: "20px 20px 0 0", padding: "18px 20px calc(22px + env(safe-area-inset-bottom))", borderTop: "1px solid rgba(255,255,255,0.08)" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
          <span style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 20, letterSpacing: 0.5, color: "#F3EFEA" }}>{headerText}</span>
        ...
        {step === "pick" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            <button onClick={() => pick("clip")} style={uploadOptionStyle(GELS.pink)}>
              <Music2 size={17} /> Upload a track clip
            </button>
            <button onClick={() => pick("set")} style={uploadOptionStyle(GELS.blue)}>
              <Headphones size={17} /> Upload a DJ set
            </button>
            <button onClick={onPodcast} style={uploadOptionStyle(GELS.amber)}>
              <Mic size={17} /> Start a podcast room
            </button>
        ...
        {step === "form" && (
          <div>
            <p style={sectionLabel}>{kind === "set" ? "SET TITLE" : "TRACK TITLE"}</p>
            <input
              autoFocus
              value={title}
              onChange={(e) => setTitle(e.target.value)}
              placeholder={kind === "set" ? "e.g. Midnight Circuit" : "e.g. Static Bloom"}
              style={{ width: "100%", background: "#201C2C", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 10, padding: "10px 14px", color: "#F3EFEA", fontFamily: "Inter", fontSize: 13.5, outline: "none", marginBottom: 12 }}
            />
            <p style={{ fontFamily: "Inter", fontSize: 11.5, color: "#948FA3", margin: "0 0 16px", lineHeight: 1.5 }}>
              This is a prototype — actual audio file upload isn't wired up yet, but this will publish to your profile{kind === "set" ? " and the Sets tab" : ""}.
            </p>
            <button onClick={publish} style={primaryBtnStyle}>Publish</button>
          </div>
        )}
        ...
        {step === "done" && (
          <div style={{ textAlign: "center", padding: "10px 0" }}>
            ...
            <p style={{ fontFamily: "Inter", fontSize: 12.5, color: "#948FA3", margin: "14px 0 16px" }}>
              "{title || (kind === "set" ? "Untitled Set" : "Untitled Clip")}" is on your profile now.
            </p>
    ...
    <button onClick={() => onTap(item)} style={{ background: "none", border: "none", cursor: "pointer", textAlign: "left", padding: 0 }}>
      <GelCard g={item.g} style={{ borderRadius: 12, height: 92, padding: 8, display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
        <div style={{ display: "flex", justifyContent: "space-between", position: "relative", zIndex: 1 }}>
          {item.kind === "live" ? <LiveBadge small /> : item.kind === "set" ? <Headphones size={13} color="rgba(13,11,20,0.6)" /> : <span />}
          {item.kind === "live" && (
...
function DiscoverScreen({ onClose, onOpenLive, onOpenSets }) {
  const [query, setQuery] = useState("");
  const [genre, setGenre] = useState("All");
  const combined = [
    ...CLIPS.map((c) => ({ ...c, kind: "clip" })),
    ...LIVES.map((l) => ({ ...l, kind: "live" })),
    ...SETS.map((s) => ({ ...s, kind: "set" })),
  ];
  ...
  const handleTap = (item) => {
    ...
    else if (item.kind === "set") onOpenSets();
    else onClose();
  };
...
function GoLiveSetup({
  title, setTitle, invited, toggleInvite, onStart, onClose,
  heading = "SET UP YOUR STREAM",
  titleLabel = "STREAM TITLE",
  subtext = "Go live together — invited artists share the stage, chat, and effects.",
  startLabel = "Go live",
}) {
  return (
    <div style={{ flex: 1, minHeight: 0, display: "flex", flexDirection: "column" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "14px 16px", flexShrink: 0 }}>
        ...
        <span style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 19, letterSpacing: 0.5, color: "#F3EFEA" }}>{heading}</span>
      </div>

      <div style={{ flex: 1, minHeight: 0, overflowY: "auto", padding: "4px 16px 16px" }} className="no-scrollbar">
        <p style={sectionLabel}>{titleLabel}</p>
        <input
          value={title}
          onChange={(e) => setTitle(e.target.value)}
      ...
      <div style={{ padding: "10px 16px calc(14px + env(safe-area-inset-bottom))", flexShrink: 0 }}>
        <button onClick={onStart} style={primaryBtnStyle}>
          {invited.length ? `${startLabel} with ${invited.length} co-host${invited.length > 1 ? "s" : ""}` : `${startLabel} solo`}
        </button>
      </div>
    </div>
  );
...
function NotesPanel({ notes, addNote }) {
  ...
  return (
    <div style={{ padding: "12px 16px" }}>
      <p style={sectionLabel}>SHOW NOTES ({notes.length})</p>
      <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 16 }}>
        {notes.map((n, i) => (
          <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: 10, padding: "9px 12px", borderRadius: 10, background: "#17141F", border: "1px solid rgba(255,255,255,0.06)" }}>
            <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, color: "#6b6578", flexShrink: 0, marginTop: 1 }}>{String(i + 1).padStart(2, "0")}</span>
            <span style={{ fontFamily: "Inter", fontSize: 13, color: "#F3EFEA", lineHeight: 1.4 }}>{n}</span>
          </div>
        ))}
      </div>
      <p style={sectionLabel}>ADD A TOPIC</p>
      <div style={{ display: "flex", gap: 8 }}>
        <input
          value={draft}
          onChange={(e) => setDraft(e.target.value)}
          onKeyDown={(e) => e.key === "Enter" && submit()}
          placeholder="e.g. how we picked the show name"
          style={{ flex: 1, background: "#201C2C", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 999, padding: "9px 14px", color: "#F3EFEA", fontFamily: "Inter", fontSize: 13, outline: "none" }}
        />
        <button onClick={submit} style={{ width: 38, height: 38, borderRadius: 999, border: "none", background: GELS.violet, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", flexShrink: 0 }}>
          <Send size={16} color="#0D0B14" />
        </button>
      </div>
    </div>
...
function PodcastRoomScreen({ title, hosts, onEnd }) {
  const [tab, setTab] = useState("chat");
  const [messages, setMessages] = useState(CHAT_SEED.slice(0, 3));
  const [draft, setDraft] = useState("");
  const [seconds, setSeconds] = useState(0);
  const [listeners, setListeners] = useState(62);
  const [handsUp, setHandsUp] = useState(LISTENERS_SEED);
  const [speaking, setSpeaking] = useState([]);
  const [notes, setNotes] = useState(["Intro — what this episode's about"]);
  const media = useLiveMedia(false);
  const chatEndRef = useRef(null);
  ...
  useEffect(() => {
    const t = setInterval(() => setListeners((v) => v + Math.floor(Math.random() * 3)), 4000);
    return () => clearInterval(t);
  }, []);
  useEffect(() => {
    chatEndRef.current?.scrollIntoView({ behavior: "smooth" });
  ...
  const bringOnStage = (name) => {
    setHandsUp((h) => h.filter((n) => n !== name));
    setSpeaking((s) => [...s, name]);
  };
  const send = () => {
    if (!draft.trim()) return;
    setMessages((m) => [...m, { user: "you", text: draft.trim() }]);
  ...
  const allSpeakers = ["you", ...hosts, ...speaking];
  ...
  return (
    <div style={{ flex: 1, minHeight: 0, display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ padding: "0 14px 10px", flexShrink: 0 }}>
        <p style={{ margin: "0 0 10px", fontFamily: "Inter", fontSize: 13, color: "#F3EFEA", fontWeight: 600, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{title}</p>
        <div style={{ display: "flex", gap: 14, flexWrap: "wrap" }}>
          {allSpeakers.map((h, i) => (
            <div key={h} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 5, width: 58 }}>
              {h === "you" ? (
                <div style={{ position: "relative", width: 52, height: 52, borderRadius: 999, overflow: "hidden", background: "#17141F", display: "flex", alignItems: "center", justifyContent: "center" }}>
                  {media.error ? (
                    <MicOff size={16} color="#948FA3" />
                  ) : (
                    <GelCard g={GRADIENTS[4]} style={{ width: "100%", height: "100%" }}>
                      <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1 }}>
                        <MicWaveform analyser={media.analyser} bars={5} height={16} color="#0D0B14" />
                      </div>
                    </GelCard>
                  )}
                  <button
                    onClick={media.toggleMic}
                    style={{ position: "absolute", bottom: -2, right: -2, width: 18, height: 18, borderRadius: 999, border: "1.5px solid #0D0B14", background: media.micOn ? GELS.violet : "#FF3B3B", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}
                  >
                    {media.micOn ? <Mic size={9} color="#0D0B14" /> : <MicOff size={9} color="#fff" />}
                  </button>
                </div>
              ) : (
                <GelCard g={GRADIENTS[(i + 1) % GRADIENTS.length]} style={{ width: 52, height: 52, borderRadius: 999, opacity: hosts.includes(h) ? 1 : 0.75 }} />
              )}
              <span style={{ fontFamily: "Inter", fontSize: 9.5, color: "#C9C4D6", textAlign: "center", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", maxWidth: 58 }}>{h === "you" ? "You" : h}</span>
            </div>
          ))}
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 5, marginTop: 10 }}>
          <Users size={11} color="#948FA3" />
          <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10.5, color: "#948FA3" }}>{fmtNum(listeners)} listening</span>
        </div>
      </div>
      ...
      <div style={{ display: "flex", gap: 6, padding: "0 14px 10px", flexShrink: 0 }}>
        {[
          ...
          { key: "listeners", label: "Raised hands", icon: Users },
          { key: "notes", label: "Notes", icon: Layers },
          ...
          <button
            ...
            style={{
              ...
              color: tab === key ? "#0D0B14" : "#C9C4D6", fontFamily: "Inter", fontWeight: 600, fontSize: 11.5,
            }}
          >
            <Icon size={13} /> {label}{key === "listeners" && handsUp.length ? ` (${handsUp.length})` : ""}
          </button>
        ))}
      </div>
      ...
      <div style={{ flex: 1, minHeight: 0, overflowY: "auto" }} className="no-scrollbar">
        ...
        {tab === "listeners" && (
          <div style={{ padding: "8px 16px" }}>
            <p style={sectionLabel}>WAITING TO SPEAK</p>
            {handsUp.length === 0 && <p style={{ fontFamily: "Inter", fontSize: 12.5, color: "#948FA3" }}>No raised hands right now.</p>}
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {handsUp.map((name) => (
                <div key={name} style={{ display: "flex", alignItems: "center", gap: 10, padding: "9px 12px", borderRadius: 10, background: "#17141F", border: "1px solid rgba(255,255,255,0.06)" }}>
                  <GelCard g={GRADIENTS[hashCode(name) % GRADIENTS.length]} style={{ width: 28, height: 28, borderRadius: 999, flexShrink: 0 }} />
                  <span style={{ flex: 1, fontFamily: "Inter", fontSize: 13, color: "#F3EFEA" }}>{name}</span>
                  <button onClick={() => bringOnStage(name)} style={{ padding: "5px 10px", borderRadius: 999, border: "none", background: GELS.violet, color: "#0D0B14", fontFamily: "Inter", fontWeight: 600, fontSize: 11.5, cursor: "pointer" }}>
                    Bring on
                  </button>
                </div>
              ))}
        ...
        {tab === "notes" && <NotesPanel notes={notes} addNote={(n) => setNotes((ns) => [...ns, n])} />}
      </div>
...
function PodcastRoomFlow({ onClose }) {
  const [step, setStep] = useState("setup");
  const [title, setTitle] = useState("Ep. — let's talk about it");
  ...
  return (
    <div style={{ position: "absolute", inset: 0, background: "#0D0B14", zIndex: 20, display: "flex", flexDirection: "column" }}>
      {step === "setup" ? (
        <GoLiveSetup
          title={title} setTitle={setTitle} invited={invited} toggleInvite={toggleInvite}
          onStart={() => setStep("live")} onClose={onClose}
          heading="SET UP YOUR PODCAST ROOM"
          titleLabel="EPISODE TITLE"
          subtext="Come together and talk live — invited co-hosts join your room, and listeners can raise a hand to join in."
          startLabel="Start recording"
        />
      ) : (
        <PodcastRoomScreen title={title} hosts={invited} onEnd={onClose} />
      )}
    </div>
  );
}
...
function useLiveMedia(wantVideo = true) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (wantVideo && videoRef.current) { videoRef.current.srcObject = stream; videoRef.current.play?.().catch(() => {}); }
        const Ctx = window.AudioContext || window.webkitAudioContext;
        const ctx = new Ctx();
...
export default function App() {
  ...
  const addUpload = ({ kind, title }) =>
    setUploads((u) => [{ id: `u${Date.now()}`, kind, title, g: GRADIENTS[u.length % GRADIENTS.length] }, ...u]);
  const handleNav = (key) => {
    setLiveItem(null);
    setScreen(key);
  };
    ...
    <>
      {screen === "feed" && (
        <FeedScreen
          ...
          onBlog={() => handleNav("blog")}
          onComment={setCommentClip}
          onRemix={setRemixClip}
          commentsState={commentsState}
        />
      )}
      {screen === "live" && !liveItem && <LiveScreen onOpen={setLiveItem} onSearch={() => setDiscoverOpen(true)} onBlog={() => handleNav("blog")} />}
      {screen === "live" && liveItem && <LiveDetailScreen item={liveItem} onBack={() => setLiveItem(null)} />}
      {screen === "sets" && <SetsScreen onSearch={() => setDiscoverOpen(true)} onBlog={() => handleNav("blog")} onStartPodcast={() => setPodcastOpen(true)} />}
      {screen === "blog" && <BlogScreen />}
      {screen === "profile" && <ProfileScreen uploads={uploads} />}
      ...
      {podcastOpen && <PodcastRoomFlow onClose={() => setPodcastOpen(false)} />}
      {commentClip && (
        <CommentsPanel
          clip={commentClip}
      ...
      {discoverOpen && <DiscoverScreen onClose={() => setDiscoverOpen(false)} onOpenLive={openLiveFromDiscover} onOpenSets={openSetsFromDiscover} />}
